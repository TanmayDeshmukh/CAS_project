...
public:
  ...
  : Node("odom_tf_publisher")
  {
    ...
    state_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("odom", 1, std::bind(&PubOdom::new_odom_callback, this, std::placeholders::_1));
    
  }
  
  void new_odom_callback(const nav_msgs::msg::Odometry::SharedPtr odom)
  {
      ...
      odom_trans_.transform.translation.x = odom->pose.pose.position.x;
      //RCLCPP_INFO(get_logger(), "setting y");
      odom_trans_.transform.translation.y = odom->pose.pose.position.y;
      odom_trans_.transform.translation.z = 0.0;
      //RCLCPP_INFO(get_logger(), "setting ori");
      odom_trans_.transform.rotation = odom->pose.pose.orientation;
